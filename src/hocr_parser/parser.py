...
class Page(HOCRElement):
    ...
    def ocr_text(self):
        ...
        for element in self._elements[:-1]:
            ...
            if len(output) > 0: 
                output += "\n"
        output += "[ T: Area,  L : " + str(_elements[-1].leftAlignedWithParent) + ", R : "+ str(_elements[-1].rightAlignedWithParent)+ "]" + self._elements[-1].ocr_text()
...
class Area(HOCRElement):
    ...
    def ocr_text(self):
        ...
        for element in self._elements[:-1]:
            ...
            output += " "
        output += "[ T: Para,  L : " + str(_elements[-1].leftAlignedWithParent) + ", R : "+ str(_elements[-1].rightAlignedWithParent)+ "]" + self._elements[-1].ocr_text()
        return output
...
class Paragraph(HOCRElement):
    ...
    def ocr_text(self):
        ...
        for element in self._elements[:-1]:
            ...
            output += " "
        output += "[ T: Line,  L : " + str(_elements[-1].leftAlignedWithParent) + ", R : "+ str(_elements[-1].rightAlignedWithParent)+ "]" +self._elements[-1].ocr_text()
        return output
